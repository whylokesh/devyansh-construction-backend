...
func main() {
	...
	advanceRepo := repository.NewAdvanceRepository(db.DB)
	...
	advanceService := service.NewAdvanceService(advanceRepo)
	...
	advanceHandler := handler.NewAdvanceHandler(advanceService)

	// Middleware
	authMiddleware := middleware.NewAuthMiddleware(cfg.JWTSecret)

	r := chi.NewRouter()
	routes.RegisterRoutes(r, userHandler, siteHandler, workerHandler, attendanceHandler, advanceHandler, authMiddleware)
